...
- Stores work with Domain Models (`{Name}ViewModel` in `features/{feature}/interfaces/`), never DTOs.
...
interface EntityUiStore {
  // State
  selected: EntityViewModel | null;
  filters: { search: string; onlyActive: boolean };
  // Actions
  setSelected: (e: EntityViewModel | null) => void;
...
| Completion | the store records completion via a plain fetcher call (e.g. add-to-history endpoint), then `queryClient.invalidateQueries({ queryKey: [...] })` for the affected list (the exported `queryClient` singleton at `common/api/query-client.ts`); recording failure must not break the flow (try/catch) |
| Imports | its own api fetcher functions, the exported `queryClient` singleton and the toast lib — **never React** |
...
| Use the exported `queryClient` singleton (`common/api/query-client.ts`) from stores for invalidation | Import React hooks into a store file |
