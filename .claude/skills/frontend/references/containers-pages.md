...
| | `toast`, navigation, any external effect (API calls) |
...
job is "confirm + one mutation" may render the MUI `Dialog` shell + its
title/description/buttons directly (with inline strings): the dialog *is* the
...
return (
  <Stack direction="row" sx={{ borderBottom: 1, bgcolor: "warning.light", px: 2, py: 1 }}>
    <WarningIcon /> <Typography variant="caption">Something needs attention</Typography> <Button>…</Button>
  </Stack>
);
```

### Shell-with-slots — how a container composes without raw markup

When a connected unit has real layout (heading + anchored dropdown + controls
row), the container can't hold that markup (Rule B) and a single mega-`<XView>`
receiving the whole view model is NOT the answer either — that just moves the
monolith. Split it:

- **`XShell`** (component): pure layout with named `ReactNode` slots
  (`input`, `dropdown`, `controls`, `status`, …). Owns every `Box`/`sx`.
- **Small basic components** per slot (`SearchInput`, `SearchControls`, …):
  each receives only the props it renders.
- **The container** calls the hook and fills the slots — real composition
  decisions (which slot renders, with which props) live here:
...
export function Search({ config }: Props) {
  ...
  return (
    <SearchShell
      ...
      input={<SearchInput value={search.searchQuery} onChange={search.onChange} … />}
      dropdown={search.showDropdown && <SearchResultsDropdown …>…</SearchResultsDropdown>}
      controls={<SearchControls searchRadius={search.searchRadius} … />}
      status={search.isGeocoding && <SearchStatus … />}
    />
  );
}
```
...
`rows: { entity, miles, isSelected }[]` — and exposes the selection handler
(`onToggle(entity)`); the item receives everything via props. An item that reads
the store/URL or calls `map.flyTo` itself is a container in disguise.
...
export const EntityListContainer = () => {
  // 'entities' is typed as EntityViewModel[] (Domain Model), NOT DTO
...
export const EntityListPage = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<EntityViewModel | null>(null);
